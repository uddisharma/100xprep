...
export function PaginationDemo({ count }: { count: number }) {
    ...
    return (
        <Pagination>
            <PaginationContent>
                <PaginationItem className={` ${page == '1' ? 'cursor-not-allowed' : 'cursor-pointer'}`}
                ...
                <PaginationItem className={` ${Number(page) == count ? 'cursor-not-allowed' : 'cursor-pointer'}`} onClick={handlePageIncrement}>
                    <PaginationNext />
                </PaginationItem>
            </PaginationContent>
        </Pagination>
