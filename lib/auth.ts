...
export const NEXT_AUTH_CONFIG = {
      ...
      async authorize(credentials: RegisterType | undefined) {
        ...
        if (existingUser) {
          if (
            ...
            existingUser.accounts[0].provider !== "credentials"
          ) {
            throw new Error(
              `Please login with your ${existingUser.accounts[0].provider} account`,
            );
          } else {
  ...
  callbacks: {
    jwt: async ({ user, token }: any) => {
      if (user) {
        token.uid = user.id;
        token.role = process.env.ADMINS?.split(",").includes(user.email)
          ? "admin"
          : "user";
        user.role = process.env.ADMINS?.split(",").includes(user.email)
          ? "admin"
          : "user";
      }
      return token;
